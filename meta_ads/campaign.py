...
from meta_ads.api import MetaAPIError


def _rollback_campaign(api, campaign_id):
    """Attempt to delete a partially-created campaign for cleanup."""
    try:
        api.delete_campaign(campaign_id)
        click.echo(click.style("  Rolled back: campaign deleted.", fg="yellow"))
    except Exception:
        click.echo(click.style(
            f"  Warning: failed to roll back campaign {campaign_id}. Delete it manually.",
            fg="red",
        ))
...
def create_full_campaign(api, config):
    ...
    # Step 1: Upload images (before creating anything on Meta)
    ...
    # Steps 3-4 wrapped in try/except for rollback on failure
    try:
        ...
        click.echo(click.style("\n  Campaign creation failed. Cleaning up...", fg="red"))
        if not api.dry_run:
            _rollback_campaign(api, campaign_id)
        raise
